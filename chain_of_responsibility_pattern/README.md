...
This can be used when you have multiple potential handlers or data sources to try in sequence. Each handler attempts to process the request; if it cannot, it passes the request to the next handler in the chain until one succeeds or the chain ends.
